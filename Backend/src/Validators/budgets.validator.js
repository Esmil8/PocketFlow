...
const createBudgetValidation = [
    body('categoria').notEmpty().withMessage('La categoria es requerida').isString().withMessage('La categoria debe ser un texto').custom((val) => {
        if (!val) return true;
    ...
    body('anio').optional().isInt({ min: 2000, max: 2100 }).withMessage('El anio debe ser un anio valido'),
    body('descripcion').optional().isLength({ max: 200 }).withMessage('La descripcion no puede tener mas de 200 caracteres')
];

const updateBudgetValidation = [
    param('id').isMongoId().withMessage('El ID del presupuesto no es valido'),
...
const getBudgetsValidation = [
    ...
    query('anio').optional().isInt({ min: 2000, max: 2100 }).withMessage('El anio debe ser un anio valido')
];
